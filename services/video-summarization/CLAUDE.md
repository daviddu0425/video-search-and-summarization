...
### Multi-Architecture Jenkins Pipeline

**Pipeline**: `ci/Jenkinsfile.develop.multiarch`

This declarative pipeline builds Docker images for AMD64 and ARM64 architectures (with platform variants) in parallel, then creates multi-architecture manifests.

**Supported Architectures**:
- `amd64`: Standard x86_64 architecture
- `arm64-sbsa`: ARM64 Server-Based System Architecture (discrete GPUs)
- `arm64-igpu`: ARM64 with integrated GPU (Jetson/embedded devices)

**Pipeline Flow**:

1. **Basics Stage** (must pass before continuing):
   - Checkout: Git clone with environment info
   - Lint: Python linting with flake8 (optional via `RUN_LINT` parameter)
   - Security Scan: Placeholder for NGC Pulse scanner (optional via `RUN_SECURITY_SCAN` parameter)

2. **Unit Tests Stage** (optional via `RUN_UNIT_TESTS` parameter):
   - Runs unit tests in Ubuntu container
   - Currently a placeholder - TODO to implement test execution

3. **Build Multi-Arch Images Stage** (optional via `RUN_BUILD` parameter):
   - Uses Jenkins matrix to build amd64, arm64-sbsa, and arm64-igpu in parallel
   - Each architecture runs in separate Kubernetes pod with Docker-in-Docker
   - Steps per architecture:
     - `get-vault-credentials`: Fetch NGC/API keys from HashiCorp Vault
     - `checkout`: Git checkout
     - `nc-dockerd`: Wait for Docker daemon to be ready
     - `base-image`: Build base image using `make -C docker/base build` (with ARM_PLATFORM env var for ARM64 variants)
     - `via-image`: Build VIA image locally using `docker buildx build --load`
     - `unit-tests`: Run tests on locally built image (AMD64 only)
     - `push-arch-image`: Push architecture-specific image to NGC (only after tests pass)
   - After all architectures complete:
     - `verify-images`: Verify all pushed images exist in NGC registry
     - `merge-images`: Creates and pushes multi-arch manifest combining all three architectures
     - `print-image-urls`: Displays all pushed image URLs

4. **Bare Metal Test Stage** (optional via `RUN_NODE_TEST` parameter):
   - Runs after all builds complete successfully
   - Reserves bare metal node with specified GPU (H100, RTX6000PROBW, RTX6000PROBW-SE)
   - Steps:
     - `get-vault-credentials-bare-metal`: Fetch credentials and SSH keys from Vault
     - `install-prerequisites-bare-metal`: Install tools (make, git, terraform, etc.)
     - `reserve-bare-metal-node`: Reserve lockable node and set up SSH access
     - `install-infra-on-bare-metal`: Install NVIDIA driver + Docker + NGC CLI using nv-one-click
     - `verify-gpu`: Run nvidia-smi to verify GPU access
     - `cleanup-infra-on-bare-metal`: Uninstall infrastructure components

**Pipeline Parameters**:
- `RUN_NODE_TEST`: Enable/disable bare metal GPU node testing (default: true)
- `BM_NODE_LABEL`: GPU type for testing - H100, RTX6000PROBW, or RTX6000PROBW-SE (default: H100)
- `RUN_BUILD`: Enable/disable Docker image builds (default: true)
- `BUILD_ARCHS`: Which architectures to build - all, amd64, arm64-sbsa, or arm64-igpu (default: all)
- `RUN_LINT`: Enable/disable Python linting (default: true)
- `RUN_UNIT_TESTS`: Enable/disable unit tests (default: false)
- `RUN_SECURITY_SCAN`: Enable/disable security scanning (default: false)

**Output Images** (VERSION = [git_tag-]commit_sha[-arch]):
```
{IMAGE_NAME}:{VERSION}-amd64            # AMD64 specific
{IMAGE_NAME}:{VERSION}-arm64-sbsa       # ARM64 SBSA specific
{IMAGE_NAME}:{VERSION}-arm64-igpu       # ARM64 iGPU specific
{IMAGE_NAME}:{VERSION}-{BUILD_SUFFIX}   # Multi-arch with build number
{IMAGE_NAME}:{VERSION}                  # Multi-arch latest
```

Example:
```
nvcr.io/nv-metropolis-dev/metropolis-analytic/vss-video-summarization:v1.2.3-abc1234-amd64
nvcr.io/nv-metropolis-dev/metropolis-analytic/vss-video-summarization:v1.2.3-abc1234-arm64-sbsa
nvcr.io/nv-metropolis-dev/metropolis-analytic/vss-video-summarization:v1.2.3-abc1234-arm64-igpu
nvcr.io/nv-metropolis-dev/metropolis-analytic/vss-video-summarization:v1.2.3-abc1234-dev123
nvcr.io/nv-metropolis-dev/metropolis-analytic/vss-video-summarization:v1.2.3-abc1234
```

**Infrastructure**:
- Kubernetes cloud: `blsm-prod-cloud`
- Service account: `met-vss-sa`
- NGC credentials stored in HashiCorp Vault
- GitLab integration for commit status reporting

**Key Functions**:
- `getDockerPodYaml(arch, includeVault)`: Generates K8s pod YAML with Docker-in-Docker for specified arch
- `getVaultContainerYaml()`: Returns YAML for Vault access container
- `parseArchitecture(combinedArch)`: Parses combined arch string (e.g., "arm64-sbsa") into base arch and platform
- `computeImageVersion(arch)`: Computes version string from git tag/commit with hyphen separators
- `fetchCredsFromVault()`: Fetches NGC_API_KEY, HF_TOKEN, OPENAI_API_KEY, NVIDIA_API_KEY, SSH keys
- `getNodeIp(label, sshKey)`: Reserves lockable bare metal node and configures SSH access
- `waitForDockerd()`: Polls until Docker daemon is ready

**Linting**: Enforced in CI using flake8==7.1.1 with flake8-black==0.3.6 and flake8-isort==6.1.1
